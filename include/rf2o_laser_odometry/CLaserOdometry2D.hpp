...
namespace rf2o
{
  ...
  {
  public:
    CLaserOdometry2D(rclcpp::Node::SharedPtr node);
    ...
    Eigen::MatrixXf &getRangeWf(); // range_wf
    const unsigned int &getWidth() const; // width
    const double &getLinSpeed() const; // lin_speed;
    const double &getAngSpeed() const; // ang_speed;
