...
namespace rf2o
{
   ...
   */
  CLaserOdometry2D::CLaserOdometry2D(rclcpp::Node::SharedPtr node) : verbose(false),
                              ...
                              const geometry_msgs::msg::Pose &initial_robot_pose)
  {
    ...
    RCLCPP_INFO(node_->get_logger(), "Got first Laser Scan .... Configuring node");
    ...
    // RCLCPP_INFO_STREAM(node_->get_logger(), "[rf2o] Setting origin at:\n"<< robot_initial_pose.matrix());
  ...
  Eigen::MatrixXf &CLaserOdometry2D::getRangeWf()
  {
    return range_wf;
  }

  const unsigned int &CLaserOdometry2D::getWidth() const
  {
    return width;
  }

  const double &CLaserOdometry2D::getLinSpeed() const
  {
    return lin_speed;
  }

  const double &CLaserOdometry2D::getAngSpeed() const
  {
    return ang_speed;
  };
  ...
  bool CLaserOdometry2D::odometryCalculation(const sensor_msgs::msg::LaserScan &scan)
  {
    ...
    auto m_runtime = node_->get_clock()->now() - start;
    RCLCPP_INFO(node_->get_logger(), "execution time (ms): %f",
  ...
  void CLaserOdometry2D::solveSystemNonLinear()
  {
    ...
    // RCLCPP_INFO_STREAM(node_->get_logger(), "[rf2o] COV_ODO:\n" << cov_odo);
  }
  ...
  bool CLaserOdometry2D::filterLevelSolution()
  {
    ...
    {
      RCLCPP_WARN(node_->get_logger(), "WARNING: Eigensolver couldn't find a solution. Pose is not updated");
      return false;
    }
  ...
  void CLaserOdometry2D::PoseUpdate()
  {
    ...
    RCLCPP_INFO(node_->get_logger(), "Laser odom [x,y,yaw]=[%f %f %f]",
    ...
    RCLCPP_INFO(node_->get_logger(), "Robot-base odom [x,y,yaw]=[%f %f %f]",
