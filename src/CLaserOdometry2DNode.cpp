...
void CLaserOdometry2DNode::init()
{
  rf2o_ref = std::make_shared<CLaserOdometry2D>(shared_from_this());
  // Init variables
...
void CLaserOdometry2DNode::LaserCallBack(const sensor_msgs::msg::LaserScan::SharedPtr new_scan)
{
  if (GT_pose_initialized)
  {
    ...
    if (rf2o_ref->first_laser_scan == false)
    {
      ...
      for (unsigned int i = 0; i < rf2o_ref->getWidth(); i++)
        rf2o_ref->getRangeWf()(i) = new_scan->ranges[i];
...
void CLaserOdometry2DNode::publish()
{
  ...
  tf_quaternion.setRPY(0.0, 0.0, rf2o::getYaw(rf2o_ref->getPose().rotation()));
  ...
  // set the position
  odom.pose.pose.position.x = rf2o_ref->getPose().translation()(0);
  odom.pose.pose.position.y = rf2o_ref->getPose().translation()(1);
  odom.pose.pose.position.z = 0.0;
  odom.pose.pose.orientation = quaternion;
  // set the velocity
  odom.child_frame_id = base_frame_id;
  odom.twist.twist.linear.x = rf2o_ref->getLinSpeed(); // linear speed
  odom.twist.twist.linear.y = 0.0;
  odom.twist.twist.angular.z = rf2o_ref->getAngSpeed(); // angular speed
  // publish the message
  ...
  if (publish_tf)
  {
    ...
    odom_trans.transform.translation.x = rf2o_ref->getPose().translation()(0);
    odom_trans.transform.translation.y = rf2o_ref->getPose().translation()(1);
...
double CLaserOdometry2DNode::getFreq() const
{
  return freq;
}
...
int main(int argc, char **argv)
{
  ...
  myLaserOdomNode->init();

  // set desired loop rate
  rclcpp::Rate rate(myLaserOdomNode->getFreq());
